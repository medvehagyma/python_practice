# Student grades counting pattern example
## User documentation


### About the program
I'm learning the counting pattern in Python and this is another example using student grades.  
There are grades in a list and I'm counting the ones that are exactly 4 or above 4.
...
` py student_grades.py `

## Developer documentation

### About the program
The program was made with Python 3.13.2. With Visual Studio Code on Windows 10. The source code is divided into 3 main parts. In order:  
1. inputs 
2. algorithm  
3. outputs 
...
input: n &isin; N, grades &isin; Z [0..n]  
output: cnt &isin; N  
precondition: &forall; i &isin; [0..n]: grades[i] &isin; [1..5]  
postcondition: cnt = $\sum_{i=0}^{n} 1 \quad \text{where } grades[i]>=4 $  
...
|A(i)|grades[i] >= 4|condition: This has to be true for an element to be counted into the result.|

![main counting pattern structogram](main_student_grades_algorithm.png)  

Output part: We simply print the resulting number to the terminal in which the program was executed.


### Testing
The program was tested with unit testing in the ` test_student_grades.py ` file using pytest which required to be installed for testing.  
The test file can be ran with the following command:  
` py -m pytest test_student_grades.py `  
